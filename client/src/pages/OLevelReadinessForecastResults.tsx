...
export default function OLevelReadinessForecastResults() {
  ...
  const subjectResults = payload.subjects.map((subject) => {
    ...
    return { subject, readinessScore, ...forecast };
  });
  ...
  return (
    <Layout>
      ...
      <section className="bg-gradient-to-r from-[#2366c9] via-[#4f86e0] to-[#2366c9] py-14 text-white">
        <div className="container-custom grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold md:text-5xl">Your Forecast Is Ready.</h1>
          ...
          <div className={`rounded-2xl border p-6 shadow-xl ${topScore.colorClass}`}>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#2366c9]">Main Forecast</p>
            <h2 className="mt-3 text-3xl font-bold">{topScore.gradeBand}</h2>
            <p className="mt-2 text-sm">Readiness score: {topScore.readinessScore}%</p>
            <p className="mt-4 text-sm leading-7">{topScore.recommendedAction}</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-14">
        <div className="container-custom grid gap-6 lg:grid-cols-[1fr_1fr]">
          <div className="rounded-2xl border border-[#dbe4ef] p-6">
            <div className="flex items-center gap-2 text-[#2366c9]"><ClipboardList className="h-5 w-5" /><p className="text-xs font-bold uppercase tracking-[0.22em]">Subject Results</p></div>
            <div className="mt-5 space-y-4">
              {subjectResults.map((result) => (
                <div key={result.subject} className="rounded-xl border border-slate-200 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="font-semibold text-[#2366c9]">{result.subject}</p>
          ...
          <div className="rounded-2xl border border-[#dbe4ef] bg-[#eef6ff] p-6">
            <div className="flex items-center gap-2 text-[#2366c9]"><Crown className="h-5 w-5" /><p className="text-xs font-bold uppercase tracking-[0.22em]">Academic Team Review</p></div>
            <div className="mt-5 rounded-xl border border-[#dbe4ef] bg-white p-5">
            ...
            <div className="mt-4 grid gap-3 text-sm text-slate-700">
              <div className="flex items-start gap-2 rounded-xl bg-white p-4"><CheckCircle2 className="mt-0.5 h-4 w-4 text-[#2366c9]" />Grade band prediction issued immediately.</div>
              <div className="flex items-start gap-2 rounded-xl bg-white p-4"><FileText className="mt-0.5 h-4 w-4 text-[#2366c9]" />Personalised roadmap added after team review.</div>
              <div className="flex items-start gap-2 rounded-xl bg-white p-4"><GraduationCap className="mt-0.5 h-4 w-4 text-[#2366c9]" />Dashboard access remains available for 24 months.</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-14">
        <div className="container-custom">
          <div className="rounded-2xl border border-[#dbe4ef] bg-[#eef6ff] p-6 shadow-sm">
            <div className="flex items-center gap-2 text-[#2366c9]"><ShieldCheck className="h-5 w-5" /><p className="text-xs font-bold uppercase tracking-[0.22em]">What Happens Next</p></div>
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              {[
                ...
                <div key={title} className="rounded-xl border border-[#dbe4ef] bg-white p-4">
                  <p className="font-semibold text-[#2366c9]">{title}</p>
      ...
      <section className="bg-white py-12">
        <div className="container-custom flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-[#dbe4ef] bg-[#eef6ff] p-6">
          <div>
            <p className="text-sm font-semibold text-[#2366c9]">Need to review the diagnostic instead?</p>
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/diagnostics" className="inline-flex items-center rounded-lg bg-[#2366c9] px-5 py-3 text-sm font-semibold text-white">
              Take Free Diagnostic <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
            <Link href="/dashboard" className="inline-flex items-center rounded-lg border border-[#2366c9] px-5 py-3 text-sm font-semibold text-[#2366c9]">
              Go to Dashboard
            </Link>
          </div>
        </div>
